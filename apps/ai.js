...
async function answer(e) {
    ...
    let res = await(await fetch(`http://xiaobapi.top/api/xb/api/xiaoai.php?&msg=${e.msg}`)).text()

    if(res)	{
        let msg = res.replace(/菲菲|小思|小爱/g,name).replace(/小爱/g, name).replace(/小米/g, '').replace(/对不起，暂不支持该功能，你和我聊点儿别的吧/g,`${name}听不明白(⇀‸↼‶)`)
        e.reply(msg)
        }
      return true
   }	
